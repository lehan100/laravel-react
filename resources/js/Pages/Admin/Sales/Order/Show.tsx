...
export default function ShowPage() {
  ...
  const displayPaymentMethodName = /cod/i.test(paymentMethodName)
    ...
    : (paymentMethodName || '---');
  const displayProvinceName = currentLocale.startsWith('ja')
  ...
  const detailRows = [
    ...
    { label: trans('hancms.sales.orders.fields.placed_at'), value: item?.placed_at || '---' },
    { label: trans('hancms.sales.orders.fields.customer_name'), value: item?.customer_name || '---' },
    { label: trans('hancms.sales.orders.fields.customer_phone'), value: item?.customer_phone || '---' },
    { label: trans('hancms.sales.orders.fields.customer_email'), value: item?.customer_email || '---' },
    { label: trans('hancms.sales.orders.fields.payment_method'), value: displayPaymentMethodName },
  ];
          ...
          <div>
              ...
              <div className="print-header-inner relative flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="print-summary-card grid gap-3 rounded-3xl bg-white/10 p-5 backdrop-blur print:min-w-[280px] print:rounded-2xl print:border print:border-slate-200 print:bg-slate-50">
                  <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-sm">
                    ...
                    <span className="font-medium text-white print:text-slate-900">{item?.placed_at || '---'}</span>
            ...
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_320px] print:grid-cols-1">
              <section className="print-section-avoid rounded-3xl border border-slate-200 bg-slate-50/70 p-6 print:rounded-2xl print:bg-white">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 print:gap-x-6 print:gap-y-4">
                  {detailRows.map((row) => (
                    <div key={row.label}>
          ...
          <div className="border border-slate-300">
            ...
            <div className="grid grid-cols-2 gap-5 border-b border-slate-300 px-5 py-4 text-sm">
              ...
              <div className="space-y-2">
                ...
                <div><span className="font-semibold">{trans('hancms.sales.orders.fields.province')}:</span> {displayProvinceName}</div>
                <div><span className="font-semibold">{trans('hancms.sales.orders.fields.ward')}:</span> {displayWardName}</div>
                <div><span className="font-semibold">{trans('hancms.sales.orders.fields.customer_address')}:</span> {item?.customer_address || '................................'}</div>
                <div><span className="font-semibold">{trans('hancms.sales.orders.print.labels.placed_date')}:</span> {item?.placed_at || '---'}</div>
              </div>
            </div>
            ...
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                ...
                <tr>
                  <td className="border border-slate-400 bg-slate-50 px-2 py-2 text-center font-semibold" colSpan={5}>{trans('hancms.sales.orders.fields.subtotal')}</td>
                ...
                <tr>
                  <td className="border border-slate-400 px-2 py-2 text-center font-semibold" colSpan={5}>{trans('hancms.sales.orders.fields.shipping_total')}</td>
                  <td className="border border-slate-400 px-2 py-2 text-right whitespace-nowrap tabular-nums">
                    {formatMoney(Number(item?.shipping_total || 0))}
                  </td>
                </tr>
                <tr>
                  <td className="border border-slate-400 bg-slate-100 px-2 py-2 text-center font-semibold uppercase" colSpan={5}>{trans('hancms.sales.orders.fields.grand_total')}</td>
            ...
            <div className="grid grid-cols-3 gap-10 border-t border-slate-300 px-5 py-6 text-center text-sm">
              <div className="space-y-2">
                ...
                <div className="text-xs text-slate-500">{trans('hancms.sales.orders.print.sign_hint')}</div>
