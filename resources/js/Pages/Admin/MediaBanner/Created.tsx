...
function CreatedPage() {
    ...
    const { langs, items, positions, config_path }: any = usePage<{
        items: MediaBanner;
    ...
    const initialTranslations = langList.reduce((result: any, lang: any) => {
        ...
        const existing = items?.translations?.[locale];
        result[locale] = {
            name: existing?.name || '',
    ...
    const { data, setData, errors, post, processing } = useForm({
        ...
        undo: items?.undo ?? 0,
        position_ids: items?.positions?.map((p: MediaPosition) => p.id) || [],
                        ...
                        {['general', 'content'].map((id) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`p-5 text-left font-medium transition-all flex justify-between items-center ${activeTab === id
