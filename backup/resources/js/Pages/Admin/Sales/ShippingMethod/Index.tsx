...
export default function IndexPage() {
  ...
  const { data, setData } = useForm({ ids: '' });
      ...
      },
      {
        ...
        renderCell: (row: any) => (
          <div className="flex flex-nowrap items-center justify-end gap-1.5">
            <EditButton href={route('shipping-methods.edit', row.id)}>{trans('hancms.button.edit')}</EditButton>
            <button
              type="button"
            ...
            >
              ...
              {row.is_active ? trans('hancms.sales.shipping_methods.actions.disable') : trans('hancms.sales.shipping_methods.actions.enable')}
            </button>
  ...
  function destroy(id: number) {
    if (confirm(trans('hancms.message.destroy', { name: trans('hancms.sales.shipping_methods.name').toLowerCase() }))) {
  ...
  function destroys() {
    if (confirm(trans('hancms.message.destroys')) && data.ids.length > 0) {
      router.delete(route('shipping-methods.destroy-many', { ids: data.ids }));
    }
  }
  ...
  const toggleStatus = (id: number) => {
    router.put(route('shipping-methods.toggle-status', id), {}, {
      preserveScroll: true,
      preserveState: true,
    });
  };
  ...
  const submitFilter = (form: HTMLFormElement) => {
    ...
    router.get(route('shipping-methods.index'), {
  ...
  return (
    <div>
      <HeaderToolbar title={trans('hancms.sales.shipping_methods.name')}>
        <CreatedButton href={route('shipping-methods.create')}>{trans('hancms.button.created')}</CreatedButton>
