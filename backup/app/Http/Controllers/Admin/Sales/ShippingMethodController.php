...
use App\Repositories\ShippingMethod\ShippingMethodRepositoryInterface as RepositoryInterface;
...
class ShippingMethodController extends MainController
{
    protected string $controllerView = 'Admin/Sales/ShippingMethod/';

    protected string $routeName = 'shipping-methods.';
    ...
    public function index(): Response
    {
        ...
        return Inertia::render($this->controllerView.'Index', [
            'items' => new ShippingMethodCollection($items),
            'filters' => [
                'search' => $search,
                'per_page' => $perPage,
            ],
        ...
        try {
            ...
            if ((int) ($params['undo'] ?? 0) === 1) {
                return Redirect::to(route($this->routeName.'index'))
                    ->with('success', __('hancms.message.success.created', ['name' => mb_strtolower(__('hancms.sales.shipping_methods.name'))]));
            }
            ...
            return Redirect::back()
                ->with('error', __('hancms.message.error.created', ['name' => mb_strtolower(__('hancms.sales.shipping_methods.name'))]));
        }
    }
    ...
    public function edit(string $id): Response|RedirectResponse
    {
        ...
        if (! $item) {
            return redirect()->route($this->routeName.'index')
                ->with('error', __('hancms.message.error.edit', ['name' => mb_strtolower(__('hancms.sales.shipping_methods.name'))]));
        }

        return Inertia::render($this->controllerView.'Edit', [
            'item' => new ShippingMethodResource($item),
        ]);
    }

    public function update(ShippingMethodRequest $request, string $id): RedirectResponse
    {
        $item = $this->mainModel->get(['id' => $id], ['task' => 'get-item']);
        if (! $item) {
            return redirect()->route($this->routeName.'index')
        ...
        try {
            ...
            if ((int) ($params['undo'] ?? 0) === 1) {
                return Redirect::to(route($this->routeName.'index'))
                    ->with('success', __('hancms.message.success.edit', ['name' => mb_strtolower(__('hancms.sales.shipping_methods.name'))]));
            }
    ...
    public function destroy(string $id): RedirectResponse
    {
        try {
            ...
            return Redirect::to(route($this->routeName.'index'))
                ->with('success', __('hancms.message.success.deleted', ['name' => mb_strtolower(__('hancms.sales.shipping_methods.name'))]));
        } catch (\Throwable $th) {
