    ...
    public function calculate(array $items, ?string $couponCode = null, ?int $orderId = null): array
    {
        ...
        if ($couponCode) {
            ...
            if ($coupon) {
                ...
                if ($coupon->max_order_amount > 0 && $subtotal > $coupon->max_order_amount) {
                    $validForOrder = false;
                }
                ...
                if ($validForOrder) {
                    if (($coupon->discount_type ?? 'percent') === 'percent') {
                        $couponDiscount = ($subtotal * ($coupon->discount_value ?? 0)) / 100;
                    } else {
                    ...
                    // Coupon discount cannot exceed subtotal - other discounts
                    $maxPossibleDiscount = max(0, $subtotal - $discountTotal);
