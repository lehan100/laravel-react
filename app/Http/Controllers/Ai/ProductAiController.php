...
class ProductAiController extends Controller
{
    public function suggestContent(Request $request): JsonResponse
    {
        ...
        if ($name === '' && $description === '' && $seoKeyword === '' && $currentContent === '') {
            return response()->json([
                'message' => __('hancms.catalog.product.ai.missing_input'),
            ], 422);
        }
        ...
        try {
            ...
            if ($content === '') {
                return response()->json([
                    'message' => __('hancms.catalog.product.ai.empty_response'),
                ], 422);
            }
            ...
            return response()->json([
                'message' => __('hancms.catalog.product.ai.failed'),
            ], 500);
        }
    }
    ...
    {
        return <<<PROMPT
Create product detail content using the data below.
...
- Include one heading section, feature bullet list, and a short closing CTA paragraph.
